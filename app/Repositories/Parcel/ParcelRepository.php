...
class ParcelRepository implements ParcelInterface {
    ...
    public function parcelEvents($id) {
        return ParcelEvent::companywise()->with(['deliveryMan','pickupman', 'transferDeliveryman', 'hub', 'user'])
        ->where(function($query)use($id){
            $query->where('parcel_id',$id);
            $query->orWhere('cancel_parcel_id',$id);
        })
    ...
    public function PickupReSchedule($id,$request){
        try {
            ...
            $pickupassignevents              = ParcelEvent::where('parcel_id',$id)->whereIn('parcel_status',[parcelStatus::PICKUP_ASSIGN,parcelStatus::PICKUP_RE_SCHEDULE])->update([
                'parcel_id'        => null,
                'cancel_parcel_id' => $id,
            ]);
    ...
    public function deliveryReschedule($id,$request){
        try {
            ...
            $deliveryManStatement                = ParcelEvent::where('parcel_id',$id)->whereIn('parcel_status',[parcelStatus::DELIVERY_MAN_ASSIGN,parcelStatus::DELIVERY_RE_SCHEDULE])->update([
                'parcel_id'        => null,
                'cancel_parcel_id' => $id,
            ]);
    ...
    public function receivedWarehouseCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::RECEIVED_WAREHOUSE ){
                $pickupAsisgn  = ParcelEvent::where(['parcel_id'=>$id,'parcel_status'=>$parcel->status])->update([
                    'parcel_id'        => null,
                    'cancel_parcel_id' => $id,
                ]);
    ...
    public function returnAssignToMerchantCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::RETURN_ASSIGN_TO_MERCHANT){
                $event = ParcelEvent::where(['parcel_id'=>$id,'parcel_status'=>$parcel->status])->update([
                    'parcel_id'        => null,
                    'cancel_parcel_id' => $id,
                ]);
    ...
    public function returnAssignToMerchantRescheduleCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::RETURN_MERCHANT_RE_SCHEDULE){
                $merchantReschedule = ParcelEvent::where(['parcel_id'=>$id,'parcel_status'=>$parcel->status])->update([
                    'parcel_id'        => null,
                    'cancel_parcel_id' => $id,
                ]);
            }
    ...
    public function PickupReScheduleCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::PICKUP_RE_SCHEDULE){
                $pickupReschedule = ParcelEvent::where(['parcel_id'=>$id,'parcel_status'=>$parcel->status])->update([
                    'parcel_id'        => null,
                    'cancel_parcel_id' => $id,
                ]);
            }
    ...
    public function deliverymanAssignCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::DELIVERY_MAN_ASSIGN ){
                ...
                $receivedByhub        = ParcelEvent::where(['parcel_id'=>$id,'parcel_status'=>ParcelStatus::RECEIVED_BY_HUB])->update([
                    'parcel_id'        => null,
                    'cancel_parcel_id' => $id,
                ]);
                if($receivedByhub){
    ...
    public function deliveryReScheduleCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::DELIVERY_RE_SCHEDULE ){
                $deliverymanReschedule = ParcelEvent::where(['parcel_id'=>$id,'parcel_status'=>$parcel->status])->update([
                    'parcel_id'        => null,
                    'cancel_parcel_id' => $id,
                ]);
            }
    ...
    public function transfertoHubCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::TRANSFER_TO_HUB ){
                $transfertohub = ParcelEvent::where(['parcel_id'=>$id,'parcel_status'=>$parcel->status])->update([
                    'parcel_id'        => null,
                    'cancel_parcel_id' => $id,
                ]);
            }
    ...
    public function receivedByHubCancel($id,$request){
        try {
            ...
            if($parcel->status == ParcelStatus::RECEIVED_BY_HUB ){
                $receivedByhub = ParcelEvent::where(['parcel_id'=>$id,'parcel_status'=>$parcel->status])->update([
                    'parcel_id'        => null,
                    'cancel_parcel_id' => $id,
                ]);
            }
