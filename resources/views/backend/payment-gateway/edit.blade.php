...
<div class="container-fluid  dashboard-content">
    ...
    <div class="row">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h2 class="pageheader-title">{{ __('payment-gateway.edit_payment_gateway') }}</h2>
                    <form action="{{route('payment-gateway.update', $gateway->id)}}"  method="POST" enctype="multipart/form-data" id="basicform">
                        @csrf
                        @method("PUT")
                        <div class="row">
                            <div class="col-12 col-md-6">
                                <div class="form-group" id="name">
                                    ...
                                    <input type="text" name="name" data-parsley-trigger="change" placeholder="{{ __('placeholder.Enter_name') }}" autocomplete="off" class="form-control" value="{{old('name', $gateway->name)}}" require>
                                    @error('name')
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="type" name="type" class="form-control @error('type') is-invalid @enderror">
                                        ...
                                        @foreach(\config('payment-gateway.gateway-type') as $key => $value)
                                            <option value="{{ $value }}" {{ ($gateway->type == $value) ? 'selected' : '' }}>{{ __('PaymentType.'.$value)}}</option>
                                        @endforeach
                                    </select>
                                    @error('type')
                                        <small class="text-danger mt-2">{{ $message }}</small>
